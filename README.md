...
4. **Das Web-Interface Design (`insane-style.css`) & Custom Components:** Das Projekt bringt ein maßgeschneidertes Design für das lokale Web-Interface und angepasste Komponenten mit.
   * **Wichtig:** Kopiere die Datei `insane-style.css` (aus dem Ordner `webserver/`) und den kompletten Ordner `custom_components/` (aus dem Ordner `ESPHome/`) in exakt denselben Ordner auf deinem Server, in dem auch deine `.yaml`-Datei liegt (meistens `/config/esphome/` oder `/homeassistant/esphome/`).
   * **Wie mache ich das?** Da das Standard-ESPHome-Dashboard keinen direkten Dateiupload für diese Ordner anbietet, nutzt du in Home Assistant am besten ein Add-on wie den **"File editor"**, **"Studio Code Server"** oder einen **"Filebrowser"**. Lade die Dateien dort hoch. Nur so bindet der Compiler das Design und das wichtige UART-Routing beim Flashen korrekt ein!
